...
include 'db.php'; // Use the same database connection file
...
if ($result->num_rows === 0) {
    ...
    $debugStmt = $conn->prepare("SELECT expires FROM password_resets WHERE token = ?");
    $debugStmt->bind_param("s", $token);
    $debugStmt->execute();
    $debugResult = $debugStmt->get_result();
    if ($debugResult->num_rows > 0) {
        $row = $debugResult->fetch_assoc();
        error_log("Token found but expired. Expires: " . $row['expires'] . ", Current time: " . date('Y-m-d H:i:s'));
        echo "Token has expired.";
    } else {
        error_log("Token not found in password_resets table: $token");
        echo "Invalid token.";
    }
    $debugStmt->close();
    exit;
...
document.getElementById('resetPasswordForm').onsubmit = function(e) {
    ...
    .then(data => {
        ...
        if (data.success) {
            ...
            loginBtn.onclick = () => window.location.href = 'user_login.html';
            responseDiv.appendChild(loginBtn);
        }
    })
    .catch(err => {
        document.getElementById('responseMessage').textContent = 'Error occurred: ' + err;
    });
};
</script>
</body>
